defmodule Waveform.Synth.Def.Parse do
  ...
  alias Waveform.Synth.Def.Parse.Ugen, as: UGParse
  ...
  def parse(
    ...
    parse_submodule({synth, i}, ugen_name, options) ||
      UGParse.parse({synth, i}, ugen_name, options)
  end

  # parse enum fn
  def parse(
        {%Synth{} = synth, i},
        {{:., _, [{:__aliases__, _, [:Enum]}, function]},
        _,
        [
          args,
          {:fn, _,
    ...
    IO.inspect({"Here", args, function, destructures, body});

    {synth, inputs} = Enum.reduce args, {synth, i}, fn {synth, i}, arg ->
      block_arguments = Enum.reduce(
        destructures,
        {synth, i},
        fn {synth, i}, {name, _, nil}=destructure ->
          parse({synth, i}, destructure)
        end)

      synth = %{synth | arguments: block_arguments}

      parse({synth, i}, body)
    end

    {%{synth | arguments: arugments}, inputs}
  end
