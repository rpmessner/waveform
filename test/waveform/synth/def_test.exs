defmodule Waveform.Synth.DefTest do
  ...
  @enum_inputs "#{@fixtures}/synths/parsed/enum_inputs.ex"
  ...
  @enum_inputs_def enum_inputs_def
  ...
  describe "Enumerable functions" do
    test "multi-output synth can be created with Enum.map" do
      assert_synthdef(
        @enum_inputs_def,
        defsynth EnumInputs, [] do
          sin_outs = Enum.map(
            [[600, 1, 4, 3],
             [440, 0, 5, 2],
             [500, 1, 6, 3],
             [200, 0, 7, 2]],
            fn [freq, phase, mul, add] ->
              %SinOsc{freq: freq, phase: phase, mul: mul, add: add}
            end
          )
          %Out{bus: 0, channels: sin_outs}
        end
      )
    end
  end
